...
//--------------------------------声明实现分界线--------------------------------
    ...
    high_float operator+(const high_float &num1, const high_float &num2)
    {
        ...
        auto back_point_add = [](const high_float &num_min, const high_float &num_max) -> high_float
        {
            ...
            if (num_min_back_point_size != num_max_back_point_size)
            {
                ...
                // 将多余的小数交给返回值
                while (iter_ != num_max.back_point.end() || iter_re_ != return_float_.back_point.end())
                {
                    (*iter_re_) = (*iter_);
                    iter_++;
                    iter_re_++;
            ...
            // 开始遍历
            while (iter_min != num_min.back_point.rend() && iter_max != num_max.back_point.rend())
            {
        ...
        if (num1.sign == num2.sign) // 只处理同符号数，异号由-减法处理
        {
            ...
            char carry = 0;
            // 如果有值,证明浮点部分有进位
            if (return_float.front_point.size() && return_float.front_point[0] != 0)
            {
                carry = 1;
            }
            ...
            // 从个位开始相加
            while (iter1 != num1.front_point.rend() && iter2 != num2.front_point.rend())
            {
    ...
    high_float operator-(const high_float &num1, const high_float &num2)
    {
        ...
        {
            // 如果同为正且被减数小
            if (num1.sign && num1 < num2)
            {
                return_float = -(num2 - num1);
            }
            // 如果同为负，且被减数绝对值大
            else if (!num1.sign && num1.abs() > num2.abs())
            {
                return_float = -((-num1) - (-num2));
            }
            // 如果同为负，且减数绝对值大
            ...
            else
            {
                ...
                // 开始计算
                while ((iter1_f != temp_num1.rend()) && (iter2_f != temp_num2.rend()))
                {
                ...
                while (iter1_i != num1.front_point.rend() && iter2_i != num2.front_point.rend())
                {
    ...
    high_float operator*(const high_float &num1, const high_float &num2)
    {
        ...
        high_float return_float;
        // 有0做乘数得0
        if (num1 == HFLOAT_ZERO || num2 == HFLOAT_ZERO)
        ...
        else
        {
            ...
            // 如果被乘数有小数部分，插入小数
            if (temp1.back_point.size() > 1 || (temp1.back_point.size() == 1 && (*temp1.back_point.begin()) != 0))
            {
            ...
            // 如果被乘数有小数部分，插入小数
            if (temp2.back_point.size() > 1 || (temp2.back_point.size() == 1 && (*temp2.back_point.begin()) != 0))
            {
            ...
            return_float.sign = ((num1.sign && num2.sign) || (!num1.sign && !num2.sign));
    ...
    high_float operator/(const high_float &num1, const high_float &num2)
    {
        ...
        temp_num2.sign = true;
        // 把除数和被除数变成整数
        if (temp_num2.back_point.size() > 1 || ((temp_num2.back_point.size() == 1) && (*(temp_num2.back_point.begin()) != 0)))
        {
    ...
    {
        return front_point.empty() && back_point.empty();
    }
    ...
    {
        // 如果整数、小数全为空
        if (front_point.size() == 0 && back_point.size() == 0)
        {
            sign = true;
            return;
        }
        ...
        else
        {
            ...
            // 对整数部分从头遍历，直到没有0为止
            while (!front_point.empty() && (*iter) == 0)
            {
                // 删除开头的全部0
